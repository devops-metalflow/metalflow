...
const (
	// ChunkTmpPath 上传分片文件临时目录
	ChunkTmpPath = "chunks"
)

// FilePartInfo 文件分片信息结构体
...
type FileMergeInfo struct {
	AddressIds string `json:"addressIds" form:"addressIds"` // 批量运行脚本时机器id集合字符串
	RemoteDir  string `json:"remoteDir" form:"remoteDir"`   // 要传输到远程机器的哪个目录下
	Runnable   bool   `json:"runnable" form:"runnable"`     // 是否传输后执行
	FilePartInfo
}

// CleanIdentifier 从文件唯一标识符中去除特殊字符, 只保留数字字母
...
// GetChunkFilenameWithoutChunkNumber 获取块文件名(不带分片编号)
...
// GetUploadRootPath 获取块文件名(不带临时目录)
...
// GetChunkRootPath 获取块文件名(不带临时目录)
...
//
//nolint:gocyclo
func (s *FilePartInfo) ValidateReq() error { //nolint:gocyclo
	filePart := s
	if filePart == nil {
